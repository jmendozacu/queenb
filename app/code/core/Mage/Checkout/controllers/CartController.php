...
class Mage_Checkout_CartController extends Mage_Core_Controller_Front_Action
{
    ...
    public function addAction()
    {
        ...
        $params = $this->getRequest()->getParams();
			...
			if($params['qty']==""):
					if(Mage::helper('core')->isModuleEnabled('Queenb_Qtymgnt')):
					$pro = Mage::getModel('catalog/product')->load($params['product']);
					...
					if(!$pro->isGrouped()):
						...
						$_blockData = $this->getLayout()->createBlock('qtymgnt/qtymgnt')->loadqtydata($groupId,$pro->getId());
						if($_blockData){
							$exp=explode(",",$_blockData);
							if (!in_array("1", $exp)) {
								Mage::getSingleton('core/session')->addError('Please select Qty..');
								$this->_redirect($pro->getUrlKey());
            					return;
							}
						}
					endif;
