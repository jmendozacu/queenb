...
class CommerceLab_GreatNews_Adminhtml_CommentController extends Mage_Adminhtml_Controller_Action
{
            ...
            try {
                foreach ($commentIds as $commentId) {
                    $model = Mage::getSingleton('clnews/comment')
                        ->load($commentId)
                        ->setCommentStatus(CommerceLab_GreatNews_Helper_Data::APPROVED_STATUS)
        ...
        if (!is_array($commentIds)) {
            ...
            try {
                foreach ($commentIds as $commentId) {
                    $model = Mage::getSingleton('clnews/comment')
                        ->load($commentId)
                        ->setCommentStatus(CommerceLab_GreatNews_Helper_Data::UNAPPROVED_STATUS)
                        ->setIsMassupdate(true)
                        ->save();
                }
                $this->_getSession()
