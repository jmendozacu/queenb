...
class CommerceLab_GreatNews_Adminhtml_CategoryController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function saveAction() {
        ...
        if ($data = $this->getRequest()->getPost()) {
            $_collection = Mage::getModel('clnews/category')->getCollection()
                ->addFieldToFilter('category_id', array('nin' => $this->getRequest()->getParam('id')))
                ->addFieldToFilter('url_key', $data['url_key']);
            if (count($_collection) > 0) {
                Mage::getSingleton('adminhtml/session')->addError(Mage::helper('clnews')->__('News Category with such url_key already exists'));
