...
class CommerceLab_GreatNews_NewsitemController extends CommerceLab_GreatNews_Controller_Action
{
    public function viewAction()
    {
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            try {
                ...
                if ((int)Mage::getStoreConfig('clnews/comments/need_confirmation')) {
                    $model->setCommentStatus(CommerceLab_GreatNews_Helper_Data::UNAPPROVED_STATUS);
                    ...
                    $model->setCommentStatus(CommerceLab_GreatNews_Helper_Data::APPROVED_STATUS);
            ...
            if ((int)Mage::getStoreConfig('clnews/comments/need_confirmation') && (Mage::getStoreConfig('clnews/comments/recipient_email')!= null) && ($model->getCommentStatus() == CommerceLab_GreatNews_Helper_Data::UNAPPROVED_STATUS) && isset($commentId)) {
                ...
                try {
                    ...
                    $mailTemplate->setDesignConfig(array('area' => 'frontend'))
                        ...
                        ->sendTransactional(
                            Mage::getStoreConfig('clnews/comments/email_template'),
        ...
        $this->getLayout()->getBlock('root')->setTemplate(Mage::getStoreConfig('clnews/news_display/newsitem_layout'));
        $this->renderLayout();
    }
    ...
    public function printAction()
    {
        ...
        $block = $this->getLayout()->createBlock('clnews/newsitem')->setTemplate('clgreatnews/news_print.phtml');
        echo $block->toHtml();
    }
    ...
    public function ajaxAction()
    {
        ...
        $defaultSortOrder = Mage::getStoreConfig('clnews/comments/sortcommentsorder');
        if (!$defaultSortOrder) {
        ...
        $collection = Mage::getModel('clnews/comment')->getCollection()
            ...
            ->setOrder('created_time ', $defaultSortOrder);
        ...
        foreach ($collection->getData() as $val) {
            ...
            $data['collection'][$k]['comment'] = nl2br($val['comment']);
            $data['collection'][$k]['status'] = $val['comment_status'];
            $data['collection'][$k]['created_time'] = Mage::helper('clnews')->formatDate($val['created_time']);
