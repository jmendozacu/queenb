...
class CommerceLab_GreatNews_Model_Check extends Mage_Core_Model_Abstract
{
    ...
    public function checkExtensions()
    {
        ...
        $magentoPlatform = CommerceLab_GreatNews_Helper_Versions::getPlatform();
        foreach ($modules as $extensionName) {
            ...
            if ($platformNode = $this->getExtensionPlatform($extensionName)) {
                $extensionPlatform = CommerceLab_GreatNews_Helper_Versions::convertPlatform($platformNode);
    ...
    {
        $coll = Mage::getModel('core/config_data')->getCollection();
        $coll->getSelect()->where("path='clnews/news_display/showrightblock'");
        foreach ($coll as $cd) {
            if ($cd->getValue() == 1) {
                $loll = Mage::getModel('core/config_data')->getCollection();
                $loll->getSelect()->where("path='clnews/news_display/showleftblock'");
                foreach ($loll as $ld) {
                    if ($ld->getValue() == 1) {
                        $ld->setValue(0)->save();
                        Mage::getSingleton('adminhtml/session')
