...
class CommerceLab_GreatNews_Helper_Parser {
    var $delim = ";";
    var $strDelim = "\"";
    var $handle = null;
    var $config = array();

    public function __construct($fp, $delim = ";", $strDelim = "\"", $configLine = false){
        $this->delim = $delim;
    ...
    public function parseRecord($buffer) {
        $buffer = trim($buffer);
    ...
    public function explode ($string,$to) {
        $res = array();
        $p = strpos($string,$to);
        if ($p !== false) {
