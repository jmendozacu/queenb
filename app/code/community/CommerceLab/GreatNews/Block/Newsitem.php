...
class CommerceLab_GreatNews_Block_Newsitem extends Mage_Core_Block_Template
{
        ...
        if ($head = $this->getLayout()->getBlock('head')) {
            ...
            if ($showBreadcrumbs && ($breadcrumbs = $this->getLayout()->getBlock('breadcrumbs'))) {
                ...
                if ($category = $newsitem->getCategory()) {
                    ...
                    'link' => Mage::getUrl(Mage::helper('clnews')->getRoute()).$category->getUrlKey().Mage::helper('clnews')->getNewsitemUrlSuffix()
                        ));
                }
            ...
            if ($newsitem->getMetaTitle()!='') {
                $head->setTitle(Mage::helper('clnews')->getMetaTitle('newsitem', null, $newsitem));
            } else {
                $head->setTitle(Mage::getStoreConfig('clnews/news_meta/metatitle'));
            }
            ...
            if ($newsitem->getMetaDescription()!='') {
                $head->setDescription(Mage::helper('clnews')->getMetaDescription('newsitem', null, $newsitem));
            } else {
    ...
    public function getBackUrl()
    {
        $newsitem = $this->getNewsItem();
        if ($category = $newsitem->getCategory()) {
            return Mage::getUrl(Mage::helper('clnews')->getRoute()).$category->getUrlKey().Mage::helper('clnews')->getNewsitemUrlSuffix();
        } else {
    ...
    {
        $width_max = Mage::getStoreConfig('clnews/news_image/fulldescr_image_max_width');
        $height_max = Mage::getStoreConfig('clnews/news_image/fulldescr_image_max_height');
