...
class Magecheckout_SecuredCheckout_Helper_Config extends Mage_Core_Helper_Abstract
{
     ...
     * Abandoned cart
     */
    const ABANDONED_CART_CONFIGUARATION = 'securedcheckout/abandoned_cart/';
    /**
     * Survey config path
     */
    const SURVEY_CONFIGUARATION = 'securedcheckout/survey/';
    ...
    {
        $pollId = $this->getSurveyConfig('survey_question', $store);
    ...
    {
        $answers = array();
        $poll    = $this->getSurveyQuestion($store);
        if ($poll && $poll->getId()) {
            $answers = Mage::getModel('poll/poll_answer')->getCollection()->addPollFilter($poll->getId());
        }
