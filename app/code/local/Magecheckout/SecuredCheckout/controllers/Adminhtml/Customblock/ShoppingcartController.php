 ...
 */
class Magecheckout_SecuredCheckout_Adminhtml_Customblock_ShoppingcartController extends Mage_Adminhtml_Controller_Action
{
    /**
     * init layout and set active for current menu
     *
     * @return Magecheckout_SecuredCheckout_Adminhtml_CheckoutpromotionController
     */
    protected function _initAction()
    {
        $this->_title($this->__('SecuredCheckout'))
            ->_title($this->__('Custom Block Rule'));
        $this->loadLayout()
            ->_setActiveMenu('securedcheckout/customblock_shoppingcart')
            ->_addBreadcrumb(
    ...
    public function editAction()
    {
        ...
        $model   = Mage::getModel('securedcheckout/customblock_shoppingcart');
        if ($rule_id) {
            $this->_title($this->__('Edit Block'));
            $model->load($rule_id);
            if (!$model->getId()) {
                Mage::getSingleton('adminhtml/session')->addError(
                    Mage::helper('securedcheckout')->__('This item no longer exists')
                );
        ...
        $this->loadLayout();
        $this->_setActiveMenu('securedcheckout/customblock_shoppingcart');
        ...
        $this->_addContent($this->getLayout()->createBlock('securedcheckout/adminhtml_customblock_shoppingcart_edit'))
            ->_addLeft($this->getLayout()->createBlock('securedcheckout/adminhtml_customblock_shoppingcart_edit_tabs'));
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('securedcheckout')->__('Rule was successfully saved')
                );
    ...
    {
        $customblockIds = $this->getRequest()->getParam('securedcheckout');
        if (!is_array($customblockIds)) {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Please select item(s)'));
        } else {
            try {
                foreach ($customblockIds as $customblockId) {
                    $customblock = Mage::getModel('securedcheckout/customblock_shoppingcart')->load($customblockId);
                    $customblock->delete();
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(
    ...
    public function massStatusAction()
    {
        ...
        if (!is_array($customblockIds)) {
            ...
            try {
                foreach ($customblockIds as $customblockId) {
                    Mage::getSingleton('securedcheckout/customblock_shoppingcart')
                        ->load($customblockId)
    ...
    public function exportCsvAction()
    {
        ...
        $content  = $this->getLayout()
            ->createBlock('securedcheckout/adminhtml_customblock_shoppingcart_grid')
            ->getCsv();
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('securedcheckout/customblock_shoppingcart');
    }
}